...
/*	File Name		:CCIEF_BASIC_SLAVE.c										*/
/*	Data			:2017/03/21													*/
/*	Version			:2.00.0														*/
/*																				*/
/*	COPYRIGHT (C) 2017 CC-Link Partner Association ALL RIGHTS RESERVED			*/
/********************************************************************************/
...
void ccief_basic_slave_get_master_info( CCIEF_BASIC_SLAVE_MASTER_INFO *pInfo )
{
	...
	pInfo->usProtocolVersion = CyclicData.usProtocolVersion;
	pInfo->ucGroupNumber = CyclicData.ucGroupNumber;
	pInfo->ulId = CyclicData.ulMasterId;
	pInfo->usParameterIdNumber = CyclicData.usParameterIdNumber;
	if ( pInfo->ulId == 0 )
	{
...
#endif
{
	...
	memcpy( &usProtocolVersion, &pucData[0], sizeof(uint16_t) );
	/* Cyclic information */
	...
	if (( CyclicData.iFirst == 0 ) || ( usParameterIdNumber != CyclicData.usParameterIdNumber ) || ( usFrameSequenceNumber == 0 ) )
	{
		...
		for ( i = 0; ( i < iTotalOccupiedStationNumber ) && ( i < CCIEF_BASIC_MAX_NUMBER_OF_OCCUPIED_STATIONS_GROUP ); i ++ )
		{
			...
			if ( CyclicData.ulId == ulSlaveId )
			{
				...
				/* Master protocol version */
				CyclicData.usProtocolVersion = usProtocolVersion;
				/* Group number */
	...
	{	/* Valid the occupied stations number */
		...
		if ( *pulSlaveId == CyclicData.ulId )
		{
			...
			{	/* Valid the cyclic data */
				...
				/* Check the unit information of the master */
				if (( CyclicData.MasterNotifyInfo.usUnitInfo & CCIEF_BASIC_UNIT_INFO_APPLICATION_RUNNING ) == CCIEF_BASIC_UNIT_INFO_APPLICATION_RUNNING )
				...
				{	/* Stop the master application */
					/* Check the output hold/clear setting */
					if (( Slave.iOutputHoldClearSetting == CCIEF_BASIC_OUTPUT_DATA_HOLD )
					 || ((( CyclicData.MasterNotifyInfo.usUnitInfo & CCIEF_BASIC_UNIT_INFO_APPLICATION_STOP_FACTOR_MASK ) == CCIEF_BASIC_UNIT_INFO_APPLICATION_STOP_FACTOR_USER )
					 &&  ( Slave.iOutputHoldClearSetting == CCIEF_BASIC_OUTPUT_DATA_HOLD_USER_STOP )))
					{	/* Output Hold */
						/* Nothig */
					}
					else
					{	/* Output Clear */
						memset( ausRWw, 0, CCIEF_BASIC_RWW_RWR_SIZE * CyclicData.iOccupiedStationNumber );
						memset( ausRY, 0, CCIEF_BASIC_RX_RY_SIZE * CyclicData.iOccupiedStationNumber );
					}
				}
			}
			else
		...
		else
		{
			...
			{	/* Output Clear */
				memset( ausRWw, 0, sizeof(ausRWw) );
				memset( ausRY, 0, sizeof(ausRY) );
			}
...
void ccief_basic_slave_disconnection( void )
{
	...
	/* Initialization of the cyclic data information */
	CyclicData.iFirst = 0;
	CyclicData.usProtocolVersion = 0;
