...
reset:
	...
	Slave.iOutputHoldClearSetting = (int)UserSlaveParameter.usOutputHoldClearSetting;
	...
	if ( UserSlaveParameter.usOutputHoldClearSetting == CCIEF_BASIC_OUTPUT_DATA_HOLD )
	{
		printf( "    Output data Hold/Clear setting:\t1 [Hold]\n" );
	}
	else if ( UserSlaveParameter.usOutputHoldClearSetting == CCIEF_BASIC_OUTPUT_DATA_HOLD_USER_STOP )
	{
		printf( "    Output data Hold/Clear setting:\t2 [Hold (User stop only)]\n" );
	}
	else
	{
		printf( "    Output data Hold/Clear setting:\t0 [Clear]\n" );
	}
...
int user_callback_parameter_get( uint16_t usId, uint16_t *pusSize, uint8_t **ppucData )
{
	...
	switch ( usId )
	{
		...
		case USER_PARAMETER_ID_CYCLIC_RESPONSE_WAIT_TIME:
			*pusSize = sizeof( uint32_t );
			*ppucData = (uint8_t*)&UserSlaveParameter.ulCyclicResponseWaitTime;
			break;
		/* Getting of the output data hold/clear setting */
		case USER_PARAMETER_ID_OUTPUT_HOLD_CLEAR_SETTING:
			*pusSize = sizeof( uint16_t );
			*ppucData = (uint8_t*)&UserSlaveParameter.usOutputHoldClearSetting;
			break;
...
int user_callback_parameter_set( uint16_t usId, uint16_t usSize, uint8_t *pucData )
{
	...
	switch ( usId )
	{
		...
		case USER_PARAMETER_ID_SUBNET_MASK:
			...
			break;
		/* Setting of the default gateway ip address */
		case USER_PARAMETER_ID_DEFAULT_GW_IP_ADDRESS:
			iResult = inet_pton( AF_INET, (char*)pucData, &ulIpaddr );
			if( iResult == 1 )
			{
			...
			break;
		/* Setting of the occupied station number */
		case USER_PARAMETER_ID_OCCUPIED_STATION_NUMBER:
			memcpy( &UserSlaveParameter.usOccupiedStationNumber, pucData, sizeof( UserSlaveParameter.usOccupiedStationNumber ) );
			break;
		/* Setting of the cyclic response wait time */
		case USER_PARAMETER_ID_CYCLIC_RESPONSE_WAIT_TIME:
			memcpy( &UserSlaveParameter.ulCyclicResponseWaitTime, pucData, sizeof( UserSlaveParameter.ulCyclicResponseWaitTime ) );
			break;
		/* Setting of the output data hold/clear setting */
		case USER_PARAMETER_ID_OUTPUT_HOLD_CLEAR_SETTING:
			memcpy( &UserSlaveParameter.usOutputHoldClearSetting, pucData, sizeof( UserSlaveParameter.usOutputHoldClearSetting ) );
			break;
	...
	while ( fgets( aucLine, sizeof( aucLine ), fp ) != NULL )
	{
		...
		switch ( iId )
		{
			...
			case USER_PARAMETER_ID_CYCLIC_RESPONSE_WAIT_TIME:
				...
				break;
			case USER_PARAMETER_ID_OUTPUT_HOLD_CLEAR_SETTING:
				pParameter->usOutputHoldClearSetting = (uint16_t)strtoul( aucData, NULL, 0 );
				/* Check the output hold/clear setting */
				if ( 2 < pParameter->usOutputHoldClearSetting )
				{
					printf( "Parameter Error: Invalid number of output hold/clear setting! %d (range: 0-2)\n", pParameter->usOutputHoldClearSetting );
					iErrCode = USER_ERR_NG;
					goto end;
				}
				break;
...
int user_parameter_file_write( char *file_path, USER_SLAVE_PARAMETER *pParameter )
{
	...
	fputs( aucLine, fp );
#ifdef _WIN32
	sprintf_s( aucLine, sizeof(aucLine), "6,%d,Output Hold/Clear Setting\n", pParameter->usOutputHoldClearSetting );
#elif __linux__
	sprintf( aucLine, "6,%d,Output Hold/Clear Setting\n", pParameter->usOutputHoldClearSetting );
#endif
	fputs( aucLine, fp );
...
void user_display_cyclic_information( void )
{
	...
	if ( ulDisplayTimerStart == 0 ) {
		...
		ulDisplayTimerEnd = ulDisplayTimerStart + USER_DISPLAY_INTERVAL_TIME;
	}
	...
	/* modified */
	{
		...
		{
			printf( "      Master Protocol Version:\t\t\t-\n" );
			printf( "      Master ID:\t\t\t\tUnknown\n" );
			printf( "      Master Group Number:\t\t\t-\n" );
			printf( "      Master Unit Info:\t\t\t\t-\n" );
			printf( "      Master Parameter ID Number:\t\t-\n" );
			printf( "      Master Time Data:\t\t\t\t-\n" );
			printf( "      Average link scan time:\t\t\t- [ms]\n" );
		}
		else
		{
			printf( "      Master Protocol Version:\t\t\t0x%04X\n", MasterInfo.usProtocolVersion );
			printf( "      Master ID:\t\t\t\t0x%08lX\n", MasterInfo.ulId );
			printf( "      Master Group Number:\t\t\t%d\n", MasterInfo.ucGroupNumber );
			printf( "      Master Unit Info:\t\t\t\t0x%04X\n", MasterInfo.NotifyInfo.usUnitInfo );
			if (( MasterInfo.NotifyInfo.usUnitInfo & CCIEF_BASIC_UNIT_INFO_APPLICATION_MASK ) == CCIEF_BASIC_UNIT_INFO_APPLICATION_RUNNING )
			{
			...
			if ( 1 < MasterInfo.usProtocolVersion )
			{
				if (( MasterInfo.NotifyInfo.usUnitInfo & CCIEF_BASIC_UNIT_INFO_APPLICATION_STOP_FACTOR_MASK ) == CCIEF_BASIC_UNIT_INFO_APPLICATION_STOP_FACTOR_USER )
				{
			...
			printf( "      Master Parameter ID Number:\t\t0x%04X\n", MasterInfo.usParameterIdNumber );
			memcpy( &llTimeData, MasterInfo.NotifyInfo.aucTimeData, sizeof( llTimeData ) );
			timer_analyze_time_data( llTimeData, &TimeData );
			printf( "      Master Time Data:\t\t\t\t%d-%02d-%02d %02d:%02d:%02d.%03d\n", TimeData.usYear, TimeData.usMonth, TimeData.usDay, TimeData.usHour,
					TimeData.usMinute, TimeData.usSecond, TimeData.usMilliseconds );
			if ( ulCyclicDataCount != 0 )
			{
				printf( "      Average link scan time:\t\t\t%01.3f [ms]\n", ((float)USER_DISPLAY_INTERVAL_TIME / ulCyclicDataCount) );
			}
			else
			{
				printf( "      Average link scan time:\t\t\t0 [ms]\n");
			}
		}
		iDataSize = ( CCIEF_BASIC_RX_RY_SIZE / sizeof( uint16_t ) ) * UserSlaveParameter.usOccupiedStationNumber;
		printf( "      Cyclic data:\n" );
		printf( "      =================================================\n" );
		...
		iDataSize = ( CCIEF_BASIC_RWW_RWR_SIZE / sizeof( uint16_t ) ) * UserSlaveParameter.usOccupiedStationNumber;
		printf( "      ================================================================\n" );
